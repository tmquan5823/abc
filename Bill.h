...
Bill::Bill(){
    ...
    string nam = to_string(date.localDate().getYear());
    if(check){
        this->mahd = ngay + thang + nam + this->mahd.append(4 - to_string(this->id).length(), '0') + to_string(this->id);
    }
    else{
        ofstream file;
        if(check_Date == date.localDate()){
...
void Bill::LuuDoanhThu(){
    ...
    if(check){
        ...
        file<<this->mahd<<" "<<this->TongTienHD()<<endl;
    }
    else{
